...
//if this component is being rendered by add-video route, then below will be rendered giving add video more properties
 const cssProperties = cssRouteProps ? cssRouteProps : {};
  ...
  return (
    <form className={`add-video-form ${cssProperties.form}`} onSubmit={e => handleSubmit(e)}>
      <div className={`${cssProperties.flex} flex-form`}>
      <div className={`${cssProperties.inputBox} inputs`}>
        ...
        <label className={`add-label  ${cssProperties.labelBox}`} htmlFor="add-video-length">Video Length:</label>
        ...
        <label className={`add-label  ${cssProperties.labelBox}`}htmlFor="add-video-youtube-name">YouTube Name:</label>
      ...
      <div className = {`directions ${cssProperties.directionsBox}`}>
        <h4>Instructions</h4>
        <div className={`instructions-text ${cssProperties.instructions}`}>{fields.addError ? fields.addError : view}</div>
      </div>
      </div>
